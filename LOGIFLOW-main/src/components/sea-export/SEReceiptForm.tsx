// src/components/sea-export/SEReceiptForm.tsx
'use client';

import React, { useEffect, useState } from 'react';
import { useForm, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { seReceiptSchema, type SEReceipt } from '@/lib/schemas/seReceiptSchema';
...
interface SEReceiptFormProps {
  initialData: SEReceipt | null;
  onSubmit: (data: SEReceipt) => void;
  onCancel: () => void;
}
...
const mockSEJobs = [
    { id: 'SEJ-25-0001', invoiceId: 'INV-SE-001', customerName: 'MERCITEX PRIVATE LIMITED' },
    { id: 'SEJ-25-0002', invoiceId: 'INV-SE-002', customerName: 'ALPHA TRADING CO.' },
];

export function SEReceiptForm({ initialData, onSubmit, onCancel }: SEReceiptFormProps) {
  const { toast } = useToast();
  const defaultValues: SEReceipt = {
    teamDate: '',
    accountInfo: {
      mode: '',
      subType: '',
  ...
  const form = useForm<SEReceipt>({
    resolver: zodResolver(seReceiptSchema),
  ...
  useEffect(() => {
    if (selectedJobNo) {
      const job = mockSEJobs.find(j => j.id === selectedJobNo);
      if (job) {
  ...
  const onFormSubmit = (data: SEReceipt) => {
    onSubmit(data);
  };
  ...
  const renderDatePicker = (fieldName: any, label: string) => (
    <div>
        ...
        {errors[fieldName.split('.')[0] as keyof SEReceipt]?.[fieldName.split('.')[1] as any] && <p className="text-destructive text-xs mt-1">{(errors[fieldName.split('.')[0] as keyof SEReceipt]?.[fieldName.split('.')[1] as any] as any).message}</p>}
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit, onFormError)} className="space-y-4 max-h-[80vh] overflow-y-auto p-2">
        ...
        <div className="flex justify-between items-start p-4 border rounded-md">
            <h2 className="text-2xl font-bold">Add SE Receipt</h2>
            <div className="w-48">
            ...
            <div className="space-y-4">
                 <Section title="Job & Invoice Linking" className="h-full">
                     <div className="grid grid-cols-3 gap-x-2 gap-y-4 text-sm">
                        <div><Label>Job #</Label><Controller name="linking.jobNo" control={control} render={({field}) => <Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue placeholder="Select Job..."/></SelectTrigger><SelectContent>{mockSEJobs.map(j => <SelectItem key={j.id} value={j.id}>{j.id}</SelectItem>)}</SelectContent></Select>}/></div>
