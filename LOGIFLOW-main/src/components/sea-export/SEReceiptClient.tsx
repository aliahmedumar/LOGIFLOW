// src/components/sea-export/SEReceiptClient.tsx
'use client';
...
import { SEReceiptForm } from './SEReceiptForm';
import type { SEReceipt } from '@/lib/schemas/seReceiptSchema';
import { format, parseISO, isValid } from 'date-fns';

const initialMockReceipts: SEReceipt[] = [];

export function SEReceiptClient() {
  const { toast } = useToast();
  const [receipts, setReceipts] = useState<SEReceipt[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedReceipt, setSelectedReceipt] = useState<SEReceipt | null>(null);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    setIsLoading(true);
    const storedReceipts = localStorage.getItem('se_receipts_mock');
    if (storedReceipts) {
  ...
  const updateMockReceiptStorage = (updatedReceipts: SEReceipt[]) => {
    setReceipts(updatedReceipts);
    localStorage.setItem('se_receipts_mock', JSON.stringify(updatedReceipts));
  };
  ...
  const handleEdit = (receipt: SEReceipt) => {
  ...
  const handleSaveReceipt = (receiptData: SEReceipt) => {
    let updatedReceipts;
    ...
    if (receiptData.id) { // Editing existing
      ...
      const newReceipt = { ...receiptData, id: `rec_mock_${Date.now()}`, createdAt: new Date().toISOString() };
    ...
    <div className="p-4 md:p-6 space-y-4">
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle>SE Receipt</CardTitle>
          ...
          {filteredReceipts.length > 0 ? (
            <Table>
             ...
             <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-lg font-medium text-muted-foreground">No SE Receipts found.</p>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-7xl">
          <DialogHeader>
            <DialogTitle>{selectedReceipt ? 'Edit SE Receipt' : 'Add SE Receipt'}</DialogTitle>
          </DialogHeader>
          <SEReceiptForm 
