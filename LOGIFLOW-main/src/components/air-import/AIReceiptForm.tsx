// src/components/air-import/AIReceiptForm.tsx
'use client';

import React, { useEffect, useState } from 'react';
import { useForm, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { aiReceiptSchema, type AIReceipt } from '@/lib/schemas/aiReceiptSchema';
...
interface AIReceiptFormProps {
  initialData: AIReceipt | null;
  onSubmit: (data: AIReceipt) => void;
  onCancel: () => void;
}
...
const mockAIJobs = [
    { id: 'AIJ-25-0001', invoiceId: 'INV-AI-001', customerName: 'Pharma Logistics' },
    { id: 'AIJ-25-0002', invoiceId: 'INV-AI-002', customerName: 'Aero Parts Global' },
];

export function AIReceiptForm({ initialData, onSubmit, onCancel }: AIReceiptFormProps) {
  const { toast } = useToast();
  const defaultValues: AIReceipt = {
    teamDate: '',
    accountInfo: {
      mode: '',
      subType: '',
  ...
  const form = useForm<AIReceipt>({
    resolver: zodResolver(aiReceiptSchema),
  ...
  useEffect(() => {
    if (selectedJobNo) {
      const job = mockAIJobs.find(j => j.id === selectedJobNo);
      if (job) {
  ...
  const onFormSubmit = (data: AIReceipt) => {
    onSubmit(data);
  };
  ...
  const renderDatePicker = (fieldName: any, label: string) => (
    <div>
        ...
        {errors[fieldName.split('.')[0] as keyof AIReceipt]?.[fieldName.split('.')[1] as any] && <p className="text-destructive text-xs mt-1">{(errors[fieldName.split('.')[0] as keyof AIReceipt]?.[fieldName.split('.')[1] as any] as any).message}</p>}
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit, onFormError)} className="space-y-4 max-h-[80vh] overflow-y-auto p-2">
        ...
        <div className="flex justify-between items-start p-4 border rounded-md">
            <h2 className="text-2xl font-bold">Add AI Receipt</h2>
            <div className="w-48">
            ...
            <div className="space-y-4">
                 <Section title="Job & Invoice Linking" className="h-full">
                     <div className="grid grid-cols-3 gap-x-2 gap-y-4 text-sm">
                        <div><Label>Job #</Label><Controller name="linking.jobNo" control={control} render={({field}) => <Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue placeholder="Select Job..."/></SelectTrigger><SelectContent>{mockAIJobs.map(j => <SelectItem key={j.id} value={j.id}>{j.id}</SelectItem>)}</SelectContent></Select>}/></div>
