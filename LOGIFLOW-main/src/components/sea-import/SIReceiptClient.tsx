// src/components/sea-import/SIReceiptClient.tsx
'use client';
...
import { SIReceiptForm } from './SIReceiptForm';
import type { SIReceipt } from '@/lib/schemas/siReceiptSchema';
import { format, parseISO, isValid } from 'date-fns';

const initialMockReceipts: SIReceipt[] = [];

export function SIReceiptClient() {
  const { toast } = useToast();
  const [receipts, setReceipts] = useState<SIReceipt[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedReceipt, setSelectedReceipt] = useState<SIReceipt | null>(null);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    setIsLoading(true);
    const storedReceipts = localStorage.getItem('si_receipts_mock');
    if (storedReceipts) {
  ...
  const updateMockReceiptStorage = (updatedReceipts: SIReceipt[]) => {
    setReceipts(updatedReceipts);
    localStorage.setItem('si_receipts_mock', JSON.stringify(updatedReceipts));
  };
  ...
  const handleEdit = (receipt: SIReceipt) => {
  ...
  const handleSaveReceipt = (receiptData: SIReceipt) => {
    let updatedReceipts;
    ...
    if (receiptData.id) { // Editing existing
      ...
      const newReceipt = { ...receiptData, id: `rec_mock_si_${Date.now()}`, createdAt: new Date().toISOString() };
    ...
    <div className="p-4 md:p-6 space-y-4">
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle>SI Receipt</CardTitle>
          ...
          {filteredReceipts.length > 0 ? (
            <Table>
             ...
             <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-lg font-medium text-muted-foreground">No SI Receipts found.</p>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-7xl">
          <DialogHeader>
            <DialogTitle>{selectedReceipt ? 'Edit SI Receipt' : 'Add SI Receipt'}</DialogTitle>
          </DialogHeader>
          <SIReceiptForm 
