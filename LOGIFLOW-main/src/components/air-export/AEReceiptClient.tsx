// src/components/air-export/AEReceiptClient.tsx
'use client';
...
import { AEReceiptForm } from './AEReceiptForm';
import type { AEReceipt } from '@/lib/schemas/aeReceiptSchema';
import { format, parseISO, isValid } from 'date-fns';

const initialMockReceipts: AEReceipt[] = [];

export function AEReceiptClient() {
  const { toast } = useToast();
  const [receipts, setReceipts] = useState<AEReceipt[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedReceipt, setSelectedReceipt] = useState<AEReceipt | null>(null);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    setIsLoading(true);
    const storedReceipts = localStorage.getItem('ae_receipts_mock');
    if (storedReceipts) {
  ...
  const updateMockReceiptStorage = (updatedReceipts: AEReceipt[]) => {
    setReceipts(updatedReceipts);
    localStorage.setItem('ae_receipts_mock', JSON.stringify(updatedReceipts));
  };
  ...
  const handleEdit = (receipt: AEReceipt) => {
  ...
  const handleSaveReceipt = (receiptData: AEReceipt) => {
    let updatedReceipts;
    ...
    if (receiptData.id) { // Editing existing
      ...
      const newReceipt = { ...receiptData, id: `rec_mock_ae_${Date.now()}`, createdAt: new Date().toISOString() };
    ...
    <div className="p-4 md:p-6 space-y-4">
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle>AE Receipt</CardTitle>
          ...
          {filteredReceipts.length > 0 ? (
            <Table>
             ...
             <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-lg font-medium text-muted-foreground">No AE Receipts found.</p>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-7xl">
          <DialogHeader>
            <DialogTitle>{selectedReceipt ? 'Edit AE Receipt' : 'Add AE Receipt'}</DialogTitle>
          </DialogHeader>
          <AEReceiptForm 
