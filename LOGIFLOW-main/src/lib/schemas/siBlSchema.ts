...
// src/lib/schemas/siBlSchema.ts
import { z } from 'zod';

const blInfoSchema = z.object({
  jobRef: z.string().min(3, "Job# must be at least 3 characters."),
  hblNo: z.string().min(1, "HBL# is required.").optional(),
  customsMbl: z.string().optional(),
  supplier: z.string().min(5, "Supplier must be at least 5 characters."),
...
const complianceSchema = z.object({
  ...
  customsValue: z.preprocess(
    (val) => (val === "" || val === undefined || val === null ? undefined : Number(val)),
    z.number().nonnegative("Customs Value cannot be negative.").optional()
  ),
});
...
export const siBlSchema = z.object({
...
export type SI_BL = z.infer<typeof siBlSchema>;
