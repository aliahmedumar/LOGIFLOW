// src/components/sea-export/SEBlClient.tsx
'use client';
...
export function SEBlClient() {
  const router = useRouter();
  const { toast } = useToast();
  const [billsOfLading, setBillsOfLading] = useState<SE_BL[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    setIsLoading(true);
    // Mock fetching data from localStorage
    const storedBls = localStorage.getItem('se_bl_mock');
    if (storedBls) {
  ...
  const handleAddNewBL = () => {
    router.push('/dashboard/sea-export/se-bl/new');
  };
  ...
  if (isLoading) {
    return <div className="p-6 text-center">Loading Bills of Lading...</div>;
  }

  return (
    <div className="p-4 md:p-6 space-y-6 relative min-h-[calc(100vh-10rem)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <h1 className="text-3xl font-bold text-foreground font-headline">SE Bill of Lading</h1>
      </div>
          ...
          <Card className="shadow-lg">
            <CardContent className="p-6">
              <div className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <p className="text-lg font-medium text-muted-foreground">No B/Ls found.</p>
                <Button onClick={handleAddNewBL} className="mt-4">
                  <Plus className="mr-2 h-4 w-4" /> Add B/L
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          billsOfLading.map((bl) => (
            <Card key={bl.id} className="shadow-lg">
              <CardHeader className="pb-3 pt-4 px-4 border-b bg-card rounded-t-lg">
                ...
                <span className="text-sm text-muted-foreground">{bl.blInfo.shipper}</span>
              </CardHeader>
