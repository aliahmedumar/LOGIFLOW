// src/components/air-import/AIInvoiceForm.tsx
'use client';

import React, { useEffect } from 'react';
import { useForm, useFieldArray, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { aiInvoiceSchema, type AIInvoice, type ChargeItem } from '@/lib/schemas/aiInvoiceSchema';
...
interface AIInvoiceFormProps {
  initialData: AIInvoice | null;
  onSubmit: (data: AIInvoice) => void;
  onCancel: () => void;
}

// Mock data, should come from a central store or API
const mockAIJobs = [
    { id: 'AIJ-25-0001', hbl: 'HAI-DXB-KHI-123', mbl: 'MAI-DXB-KHI-456', costCenter: 'HEAD OFFICE', customerName: 'Pharma Logistics' },
    { id: 'AIJ-25-0002', hbl: 'HAI-JFK-LHE-789', mbl: 'MAI-JFK-LHE-012', costCenter: 'BRANCH A', customerName: 'Aero Parts Global' },
];
const mockCustomers = [
    { name: 'Pharma Logistics', bankDetail: 'Bank Al-Habib, Main Branch, Account # 123456789' },
...
const mockChargeDescriptions = ['Air Freight', 'Clearance Fee', 'Security Surcharge', 'Handling Fee'];

export function AIInvoiceForm({ initialData, onSubmit, onCancel }: AIInvoiceFormProps) {
  const defaultValues: AIInvoice = {
  ...
  const form = useForm<AIInvoice>({
    resolver: zodResolver(aiInvoiceSchema),
  ...
  useEffect(() => {
    if(selectedJobId) {
      const job = mockAIJobs.find(j => j.id === selectedJobId);
      if(job) {
  ...
  const onFormSubmit = (data: AIInvoice) => {
    onSubmit(data);
  };
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4 max-h-[80vh] overflow-y-auto p-2">
      ...
      {/* Details Section */}
       <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 p-4 border rounded-lg">
        ...
        <div><Label htmlFor="jobId">Job #*</Label><Controller name="jobId" control={control} render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue placeholder="Select Job"/></SelectTrigger><SelectContent>{mockAIJobs.map(j => <SelectItem key={j.id} value={j.id}>{j.id}</SelectItem>)}</SelectContent></Select>)} />{errors.jobId && <p className="text-destructive text-xs mt-1">{errors.jobId.message}</p>}</div>
