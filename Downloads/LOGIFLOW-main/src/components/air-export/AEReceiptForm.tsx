// src/components/air-export/AEReceiptForm.tsx
'use client';

import React, { useEffect, useState } from 'react';
import { useForm, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { aeReceiptSchema, type AEReceipt } from '@/lib/schemas/aeReceiptSchema';
...
interface AEReceiptFormProps {
  initialData: AEReceipt | null;
  onSubmit: (data: AEReceipt) => void;
  onCancel: () => void;
}
...
const mockCustomers = [
    ...
    { name: 'Aero Parts Global' },
];
const mockAEJobs = [
    { id: 'AEJ-25-0001', invoiceId: 'INV-AE-001', customerName: 'Tech Innovations Inc.' },
    { id: 'AEJ-25-0002', invoiceId: 'INV-AE-002', customerName: 'Aero Parts Global' },
];

export function AEReceiptForm({ initialData, onSubmit, onCancel }: AEReceiptFormProps) {
  const { toast } = useToast();
  const defaultValues: AEReceipt = {
    teamDate: '',
    accountInfo: {
      mode: '',
      subType: '',
  ...
  const form = useForm<AEReceipt>({
    resolver: zodResolver(aeReceiptSchema),
  ...
  useEffect(() => {
    if (selectedJobNo) {
      const job = mockAEJobs.find(j => j.id === selectedJobNo);
      if (job) {
  ...
  const onFormSubmit = (data: AEReceipt) => {
    onSubmit(data);
  };
  ...
  const renderDatePicker = (fieldName: any, label: string) => (
    <div>
        ...
        {errors[fieldName.split('.')[0] as keyof AEReceipt]?.[fieldName.split('.')[1] as any] && <p className="text-destructive text-xs mt-1">{(errors[fieldName.split('.')[0] as keyof AEReceipt]?.[fieldName.split('.')[1] as any] as any).message}</p>}
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit, onFormError)} className="space-y-4 max-h-[80vh] overflow-y-auto p-2">
        ...
        <div className="flex justify-between items-start p-4 border rounded-md">
            <h2 className="text-2xl font-bold">Add AE Receipt</h2>
            <div className="w-48">
            ...
            <div className="space-y-4">
                 <Section title="Job & Invoice Linking" className="h-full">
                     <div className="grid grid-cols-3 gap-x-2 gap-y-4 text-sm">
                        <div><Label>Job #</Label><Controller name="linking.jobNo" control={control} render={({field}) => <Select onValueChange={field.onChange} value={field.value}><SelectTrigger><SelectValue placeholder="Select Job..."/></SelectTrigger><SelectContent>{mockAEJobs.map(j => <SelectItem key={j.id} value={j.id}>{j.id}</SelectItem>)}</SelectContent></Select>}/></div>
